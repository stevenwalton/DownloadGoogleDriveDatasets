...
def extract(zip_file, path):
    ...
    cmd = f"7z x {path}{zip_file} -o{path}"
    return_code = Popen(cmd, shell=True).wait()
    if return_code != 0:
...
def CelebA(args):
    ...
    download_parallel(IMGS['anno_names'],
                      IMGS['anno_ids'],
                      "Anno",
                      "Annotations",
                      args)
    makedir(f"{args.directory}/Eval")
    download_parallel(IMGS['eval_names'],
                      IMGS['eval_ids'],
                      "Eval",
                      args)
    download_parallel("Readme.md",
                      IMGS['readme_id'],
                      ".",
                      args)
